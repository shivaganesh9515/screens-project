...
interface PlaylistItem { id: string; position: number; duration_ms: number; media_items: MediaItem; }
interface Playlist { id: string; name: string; playlist_items: PlaylistItem[]; }

function SortableItem({ item, onRemove, onDurationChange }: { item: PlaylistItem; onRemove: () => void; onDurationChange: (ms: number) => void }) {
...
export function PlaylistBuilder({ playlist, mediaItems, orgId }: { playlist: Playlist; mediaItems: MediaItem[]; orgId: string }) {
  ...
  const handleSave = async () => {
    ...
    const inserts = items.map((item, i) => ({ playlist_id: playlist.id, media_item_id: item.media_items.id, position: i, duration_ms: item.duration_ms }));
      ...
      <div className="flex items-center justify-between">
        ...
        <div className="flex items-center gap-2">
          <Dialog open={addOpen} onOpenChange={setAddOpen}>
            <DialogTrigger><Button variant="outline" className="rounded-xl gap-2" type="button"><Plus className="h-4 w-4" /> Add Items</Button></DialogTrigger>
        ...
        {items.length === 0 ? (
          ...
          <DndContext sensors={sensors} collisionDetection={closestCenter} onDragEnd={handleDragEnd}>
            <SortableContext items={items.map((i) => i.id)} strategy={verticalListSortingStrategy}>
              <div className="space-y-2">{items.map((item) => (<SortableItem key={item.id} item={item} onRemove={() => handleRemove(item.id)} onDurationChange={(ms) => handleDurationChange(item.id, ms)} />))}</div>
            </SortableContext>
          </DndContext>
        )}
      </div>
